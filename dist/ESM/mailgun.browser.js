// mailgun.js v13.3.0 Copyright (c) 2026 Mailgun and contributors
...
class AxiosProvider {
    ...
    makeHeadersFromObject(headersObject = {}) {
        ...
        requestHeaders = Object.entries(headersObject)
            .reduce((headersAccumulator, currentPair) => {
...
class IpPoolsClient {
    ...
    async list() {
        const response = await this.request.get('/v3/ip_pools');
        return {
            status: response.status,
            ...response.body
        };
    ...
    async create(data) {
        const response = await this.request.postWithFD('/v3/ip_pools', data);
        return {
            status: response.status,
            ...response.body
        };
    ...
    async update(poolId, data) {
        const response = await this.request.patchWithFD(`/v3/ip_pools/${poolId}`, data);
        return {
            status: response.status,
            ...response.body
        };
    ...
    async linkedDomains(poolId, query) {
    ...
    async addIp(poolId, ip) {
        const response = await this.request.put(`/v3/ip_pools/${poolId}/ips/${ip}`);
        return {
            status: response.status,
            ...response.body
        };
    ...
    async removeIp(poolId, ip) {
        const response = await this.request.delete(`/v3/ip_pools/${poolId}/ips/${ip}`);
        return {
            status: response.status,
            ...response.body
        };
    ...
    async delegate(poolId, subAccountId) {
        const response = await this.request.putWithFD(`/v3/ip_pools/${poolId}/delegate`, { subaccount_id: subAccountId });
        return {
            status: response.status,
            ...response.body
        };
    ...
    async addIps(poolId, ips) {
        const response = await this.request.post(`/v3/ip_pools/${poolId}/ips.json`, { ips });
        return {
            status: response.status,
            ...response.body
        };
    ...
    async removeIpFromDomainPool(domain, ip, replacementIp) {
        ...
        if (replacementIp && (!this.isValidIp(replacementIp) || replacementIp === 'shared')) {
            throw APIError.getUserDataError('Invalid replacement IP address', `The provided replacement IP address "${replacementIp}" is not a valid IPv4 or IPv6 address.`);
        }
        ...
        const response = await this.request.delete(`/v3/domains/${encodeURIComponent(domain)}/pool/${encodedIp}`, undefined, query);
        return {
            status: response.status,
            ...response.body
        };
    ...
    async removeDomainPool(domain) {
        const response = await this.request.delete(`/v3/domains/${encodeURIComponent(domain)}/pool/all`);
        return {
            status: response.status,
            ...response.body
        };
...
class MailingListsClient extends NavigationThruPages {
    ...
    validateQuery(query) {
        ...
        const allowedQueryKeys = ['limit', 'page'];
        const invalidKeys = Object.keys(query).filter((key) => !allowedQueryKeys.includes(key));
        if (invalidKeys.length) {
            throw APIError.getUserDataError('Unknown query key', `"lists.list": Unknown query key(s) ${invalidKeys.join(', ')}. Allowed keys are [${allowedQueryKeys.join(', ')}]`);
        }
    }
...
class MailListsMembers extends NavigationThruPages {
    ...
    async upload(mailingListAddress, file, subscribed = true, upsert = true) {
        const data = {
    ...
    parseListResponse(response) {
        const parsedResponse = {
            ...
            items: response.body.items.map((item) => {
                const responseItem = { ...item, '@timestamp': new Date(item['@timestamp']) };
                return responseItem;
            }),
...
class TagsClient {
    ...
    parseListResponse(responseBody) {
        const items = responseBody.items.map((tagInfo) => ({
            ...tagInfo,
...
class CustomMessageLimitClient {
    request;
    path;
    constructor(request) {
        this.request = request;
...
class AccountManagementClient {
    ...
    async updateAccountSettings(settingsObj) {
    ...
    // Get webhook signing key saved on the account
    async getSandboxAuthorizedRecipients() {
    ...
    async addSandboxAuthorizedRecipient(email) {
    ...
    async removeSandboxAuthorizedRecipient(email) {
    ...
    async updateAccountFeature(data) {
        const reqData = Object.entries(data).reduce((acc, currentValue) => {
            ...
            try {
                ...
                throw APIError.getUserDataError(error.message, `The "${key}" property expects JS object value with "enable" property in it. See AccountFeatureInput type.`);
            }
        }, {});
